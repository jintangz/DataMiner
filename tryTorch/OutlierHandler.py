...
if __name__ == '__main__':
    ...
    # ts = ThreeSigmaOutlierRecognizer(['col1', 'col3'])
    ...
    df = pd.read_csv(r'D:\DataMining\financial_default\train.csv')
    features = ['loanAmnt', 'interestRate', 'installment',
                'annualIncome', 'dti', 'openAcc', 'revolBal', 'revolUtil',
                'totalAcc',
                ]
    ts = ThreeSigmaOutlierRecognizer(features)
    ts.fit_transform(df)
